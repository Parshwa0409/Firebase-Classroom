...
def main(user_dict=None):
        ...
        if file:
            ...
            if confirm:
                try:
                    ...
                    messagebox.showinfo(
                        title="SUCCESS",
                        message=f"\"{file_name}\"\nSuccessfully Uploaded.\nPLEASE WAIT TILL THE STUDENTS ARE INFORMED VIA EMAIL."
                    )
                    file_link = storage.child(final_upload_path).get_url(None)
                    DB_REALTIME.send_emails(file_link=file_link, teacher_name=user_details[NAME_KEY], file_path=final_upload_path)
                    messagebox.showinfo(
                        title="EMAIL SENT",
                        message=f"EMAIL HAS BEEN SENT TO ALL THE STUDENTS SUCCESSFULLY."
                    )
    ...
    def download_solution():
        ...
        # TODO : Confirm if that file is present in the storage.
    ...
    def download_file():
        ...
        if len(directory_path) == 0:
            messagebox.showwarning(
                ...
                message='Please choose the path/location where to download the file.'
            )
    ...
    Label(
        ...
        text=f"Name : {str(user_details[NAME_KEY]).title()}\nEmail - ID: {user_details[EMAIL_KEY]}",
    ...
    upload_frame.grid(row=1, pady=8)
    ...
    download_frame.grid(row=2, padx=16, pady=8)
